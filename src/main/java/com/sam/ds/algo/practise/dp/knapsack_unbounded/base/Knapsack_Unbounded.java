package com.sam.ds.algo.practise.dp.knapsack_unbounded.base;
...
public class Knapsack_Unbounded {
    ...
    static class Recursive {
        static int unboundedKnapsack(int[] wt, int[] val, int cap, int n) {
            ...
            return max(unboundedKnapsack(wt, val, cap, n-1),
    ...
    static class Memoize {
        static int unboundedKnapsack(int[] wt, int[] val, int cap, int n, int[][] dp) {
            ...
            if (wt[n-1] > cap)
                return dp[cap][n] = unboundedKnapsack(wt, val, cap, n-1, dp);
            return dp[cap][n] = max(unboundedKnapsack(wt, val, cap, n-1, dp),
