package com.sam.ds.algo.practise.dp.knapsack.base;

/**
 * Given :: Items : wt[], val[], size = n
 *          Capacity = C
 * Maximize Value taken in knapsack
 */
public class Knapsack_01 {
    static private class Recursive {
    ...
    static private class Memoize {
        static int knapsack(int[] wt, int[] val, int C, int n, int[][] dp) {
            ...
            if (wt[n - 1] > C) {
                ...
                return dp[C][n] = Math.max(val[n-1] + knapsack(wt, val, C - wt[n-1], n - 1, dp), knapsack(wt, val, C, n-1, dp));
            }
        }
    }

    static private class DP {
        static int knapsack(int[] wt, int[] val, int C, int n) {
