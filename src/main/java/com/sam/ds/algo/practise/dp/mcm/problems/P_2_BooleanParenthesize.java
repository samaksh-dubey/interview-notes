                ...
                for (int left = 0, right = left + d; right < N; left++, right++) {
                    ...
                    for (int k = left; k < right; k++) {
                        if (operator[k] == '&') {
                            ...
                            F[left][right] += T[left][k] * F[k + 1][right] + F[left][k] * T[k + 1][right] + F[left][k] * F[k + 1][right];
                        } else if (operator[k] == '|') {
                            F[left][right] += F[left][k] * F[k + 1][right];
                            T[left][right] += T[left][k] * F[k + 1][right] + F[left][k] * T[k + 1][right] + T[left][k] * T[k + 1][right];
                        } else if (operator[k] == '^') {
