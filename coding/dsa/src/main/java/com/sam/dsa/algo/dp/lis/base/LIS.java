package com.sam.dsa.algo.dp.lis.base;
  ...
  static class Recursive {
    ...
    static int _lis(int[] A, int n) {
      ...
      for (int i = 1; i < n; i++) {
        ...
        if (A[i - 1] < A[n - 1] && lis_till_i_incl + 1 > max_here) max_here = lis_till_i_incl + 1;
      }
  ...
  static class Memoize {
    ...
    static int _lis(int[] A, int n, int[] dp) {
      ...
      for (int i = 1; i < n; i++) {
        ...
        if (A[i - 1] < A[n - 1] && lis_till_i_incl + 1 > dp[n - 1]) dp[n - 1] = lis_till_i_incl + 1;
      }
