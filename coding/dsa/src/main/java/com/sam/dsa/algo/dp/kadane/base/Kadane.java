package com.sam.dsa.algo.dp.kadane.base;
...
public class Kadane {
  ...
  static class Memoize {
    static int maxSumSubArray(int[] A, int n, int[] dp) {
      if (n == 0) return dp[n] = Math.max(0, A[n]);
  ...
  public static class DP {
    public static int maxSumSubArray(int[] A) {
      ...
      int[] dp = new int[N];
      for (int n = 0; n < N; n++) {
        if (n == 0) dp[n] = Math.max(0, A[n]);
        else {
          dp[n] = Math.max(0, A[n] + dp[n - 1]);
        }
      }
      int max = IntStream.of(dp).max().getAsInt();
      if (Arrays.stream(A).max().getAsInt() < 0) max = Arrays.stream(A).max().getAsInt();
      return max;
    }
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        Arrays.toString(getMaxSumSubArray(new int[] {-2, 1, -3, 4, -1, 2, 1, -5, -10, 4, 2})));
  }
}
