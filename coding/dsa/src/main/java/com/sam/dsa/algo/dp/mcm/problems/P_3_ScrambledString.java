package com.sam.dsa.algo.dp.mcm.problems;
    ...
    static boolean isScramble(String S1, String S2) {
      ...
      for (int i = 1; i < n; i++) {
        ...
        if (isScramble(S1.substring(n - i, n), S2.substring(0, i))
            && isScramble(S1.substring(0, n - i), S2.substring(i, n))) return true;
      }
      return false;
    }
  ...
  static class DP {
    static boolean isScramble(String S1, String S2, Map<String, Map<String, Boolean>> map) {
      ...
      for (int i = 1; i < n; i++) {
        ...
        if (calculate(S1.substring(0, i), S2.substring(0, i), map)
            && calculate(S1.substring(i, n), S2.substring(i, n), map)) return true;

        if (calculate(S1.substring(n - i, n), S2.substring(0, i), map)
            && calculate(S1.substring(0, n - i), S2.substring(i, n), map)) return true;
      }
      return false;
    }
