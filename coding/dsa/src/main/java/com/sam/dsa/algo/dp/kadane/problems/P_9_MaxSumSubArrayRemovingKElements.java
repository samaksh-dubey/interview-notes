...
public class P_9_MaxSumSubArrayRemovingKElements {
  ...
  static class Memoize {
    static int maxSumSubArray(int[] arr, int n, int k, int[][] dp) {
      if (n == 0) return dp[n][k] = Math.max(0, arr[n]);

      if (dp[n][k] != -1) return dp[n][k];

      int including = Math.max(0, arr[n] + maxSumSubArray(arr, n - 1, k, dp));

      if (k == 0) return dp[n][k] = including;
  ...
  static class DP {
    static int maxSumSubArray(int[] arr, int K) {
      ...
      for (int n = 0; n < N; n++) {
        for (int k = 0; k < K + 1; k++) {
          if (n == 0) dp[n][k] = Math.max(0, arr[0]);
          else {
      ...
      if (Arrays.stream(arr).max().getAsInt() < 0) max = Arrays.stream(arr).max().getAsInt();
