package com.sam.dsa.algo.dp.lis.problems;
...
public class P_4_BoxStacking {
  ...
  static Box[] generateRotations(Box[] boxes) {
    ...
    for (int i = 0; i < boxes.length; i++) {
      rotations[3 * i] =
          Box.of(boxes[i].h, Math.max(boxes[i].w, boxes[i].d), Math.min(boxes[i].w, boxes[i].d));
      rotations[3 * i + 1] =
          Box.of(boxes[i].w, Math.max(boxes[i].h, boxes[i].d), Math.min(boxes[i].h, boxes[i].d));
      rotations[3 * i + 2] =
          Box.of(boxes[i].d, Math.max(boxes[i].w, boxes[i].h), Math.min(boxes[i].w, boxes[i].h));
    }
    return rotations;
  }
  ...
  static int maxHeight(Box[] boxes) {
    ...
    for (int n = 1; n <= N; n++) {
      dp[n - 1] = boxes[n - 1].h;
      for (int i = 1; i <= n; i++) {
        if (boxes[n - 1].w < boxes[i - 1].w
            && boxes[n - 1].d < boxes[i - 1].d
            && dp[i - 1] + boxes[n - 1].h > dp[n - 1]) dp[n - 1] = dp[i - 1] + boxes[n - 1].h;
      }
    }
  ...
  public static void main(String[] args) {
    Box[] boxes = new Box[] {Box.of(4, 6, 7), Box.of(1, 2, 3), Box.of(4, 5, 6), Box.of(10, 12, 32)};
