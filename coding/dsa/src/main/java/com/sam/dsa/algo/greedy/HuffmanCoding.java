package com.sam.dsa.algo.greedy;
  ...
  static class Huffman {
    ...
    private Node buildTree(String s) {
      ...
      while (queue.size() != 1) {
        ...
        queue.add(
            Node.of(
                null, (left != null ? left.fq : 0) + (right != null ? right.fq : 0), left, right));
      }
    ...
    private String encodeText(String text) {
      ...
      for (Character ch : text.toCharArray()) sb.append(huffmanCode.get(ch));
      return sb.toString();
    }
    ...
    private int decodeText(Node node, int index, StringBuilder sb) {
      ...
      index++;
      index =
          encodedText.charAt(index) == '0'
              ? decodeText(node.left, index, sb)
              : decodeText(node.right, index, sb);
      return index;
    }
    ...
    public String getDecodedText() {
      ...
      if (this.huffmanTree.left == null && this.huffmanTree.right == null) {
        ...
        int index = -1;
        while (index < encodedText.length() - 1) index = decodeText(this.huffmanTree, index, sb);
      }
      return sb.toString();
    }
  }
