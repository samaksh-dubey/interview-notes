...
public class P_3_CountSubsetWithSum {
    ...
    static class Memoize {
        static int countSubsetSum(final int[] arr, final int N, final int SUM, final int[][] dp) {
            ...
            if (arr[N-1] > SUM)
                return dp[SUM][N] = countSubsetSum(arr, N - 1, SUM, dp);
            return dp[SUM][N] = countSubsetSum(arr, N - 1, SUM, dp) + countSubsetSum(arr, N - 1, SUM - arr[N - 1], dp);
        }
    }
            ...
            for (int s = 0; s < SUM + 1; s++) {
                for (int n = 0; n < N + 1; n++) {
                    if (s == 0 && n == 0)
                        ...
                        dp[s][n] = dp[s][n-1] + dp[s-arr[n-1]][n-1];
                }
            }
            return dp[SUM][N];
        }
